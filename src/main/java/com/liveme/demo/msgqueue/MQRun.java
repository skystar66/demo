...
import com.liveme.demo.masterwork.Master;
import com.liveme.demo.masterwork.Worker;
...
import com.liveme.demo.msgqueue.room.RoomsDispatcher;
...
////
     ...
     * 生成queue
     */
    public static ConcurrentLinkedQueue<LiveRoom> createQueue() {
        ConcurrentLinkedQueue<LiveRoom> queue = MQProvider.getFromRPCRoomMsgQueueByRandom();
...
//            /**没个直播间发送1000条消息*/
//            for (int j = 0; j < msgCount; j++) {
//                MQProvider.push(String.valueOf(i), String.valueOf(j));
//            }
//        }
//
...
//        //30个线程消费消息
//        for (int i = 0; i < threadPoolCount; i++) {
//            threadPool.submit(new MQConsumer2());
//        }
//        threadPool.shutdown();
//
//        while (true) {
...
//    }
    ...
    public static void moniRoomMsg4() {
        ...
        for (int i = 0; i < roomCount; i++) {
            ...
            /**没个直播间发送1000条消息*/
            for (int j = 0; j < msgCount; j++) {
                RoomsDispatcher.getInstance().add(String.valueOf(i),""+j);
                MQProvider.push(room, String.valueOf(j));
            }
        }
        ...
        //30个线程消费消息
        for (int i = 0; i < threadPoolCount; i++) {
            threadPool.execute(new MQConsumer3(
                    i % queueCount));
        }
        System.out.println("创建线程耗费时间："+(System.currentTimeMillis()-start)+"ms");
        threadPool.shutdown();
...
////
//        for (int i = 0; i < 1; i++) {
//            Thread thread = new Thread(new MQConsumer(map.get(i)));
//            thread.start();
